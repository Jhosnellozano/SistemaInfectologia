...
$(function () {
    data_list = $('#data_list').DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                ...
                orderable: false,
                render: function (data, type, row) {   
                    let buttons = '<a href="/hospital/doctor/editar/' + row.id + '/" class="btn btn-warning btn-xs"><i class="fas fa-edit"></i></a> ';
    ...
    $('#data_list tbody').on('click', 'a[rel="delete"]', function () {
        ...
        submit_with_ajax(url, 'Notificación', '¿Estas seguro de eliminar al doctor ' + '<b style="color: #304ffe;">' +  data.full_name + ' / ' + data.cedula + '</b>?', parameters, function () {
